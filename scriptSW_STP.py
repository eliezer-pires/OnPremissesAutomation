...
def swDIST_stp (vlanx):
    ...
    config_commands = ['spanning-tree mode rapid-stp',
                       'spanning-tree vlan {} priority {}'.format(vlanx).format(prior),
                       ]
    return config_commands

def swACCESS_stp (vlanx):
    global config_commands
    config_commands = ["spanning-tree vlan {} priority {}".format(vlanx).format(prior),
...
if switch[0].lower() == 'd':
    ...
    print('Se o Switch nao e de distribuição ou de acesso, este script nao vai funcionar\n')
print('Fim do Script')